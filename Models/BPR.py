...
class BPR_max(nn.Module):
    ...
    def __init__(self, user_count, item_count, dim, gpu):
        ...
        self.user_emb = nn.Embedding(self.user_count, dim, max_norm=30)
        self.item_emb = nn.Embedding(self.item_count, dim, max_norm=30)
    ...
    def forward_multi_items(self, user, item, span_required=True):
        # user : bs x 1
        # item : bs x k
        #print("rd", user.size(), item.size())
        if span_required:
