...
class PIW_LWCKD(nn.Module):
    def __init__(self, base_model, LWCKD_flag, PIW_flag, 
                 temperature = 5., num_cluster = 10, dim = 64,  gpu = None):
        super().__init__()
    ...
    def forward(self, mini_batch):
        ...
        # LWCKD + PIW
        UI_loss = torch.tensor(0).to(self.gpu)
        IU_loss = torch.tensor(0).to(self.gpu)
        UU_loss = torch.tensor(0).to(self.gpu)
        II_loss = torch.tensor(0).to(self.gpu)
        cluster_loss = torch.tensor(0).to(self.gpu)
        ...
        if self.LWCKD_flag:
            ...
            batch_before_user_emb = self.before_user_emb[map_common_batch_user]
            batch_before_item_emb = self.before_item_emb[map_common_batch_item]
            
            present_common_user_emb =  self.base_model.user_emb.weight[self.before_user_ids]
            present_common_item_emb = self.base_model.item_emb.weight[self.before_item_ids]
            ...
            if self.PIW_flag:
                batch_present_user_emb = self.base_model.user_emb.weight[common_batch_user]
                before_G = self.get_G(batch_before_user_emb, self.cluster)
                present_G = self.get_G(batch_present_user_emb, self.cluster)
            ...
            cluster_loss = self.get_cluster_loss()
    ...
    def update(self, 
               ...
               UU = None, II = None,
               present_user_ids = None, present_item_ids = None):
    ...
    def get_cluster_loss(self):
        present_item_emb = self.base_model.item_emb.weight[self.present_item_ids]
        present_item_emb = present_item_emb.detach()
        present_item_emb.requires_grad = False
        
        Q = torch.pow(1 + (torch.sum((present_item_emb.unsqueeze(1) - self.cluster)**2, dim = -1) / self.v), -((self.v + 1)/2))
